...
resource "aws_cloudwatch_metric_alarm" "nodes_green_cpu_threshold" {
  ...
  dimensions = {
    ...
    AutoScalingGroupName = aws_eks_node_group.green.resources[0].autoscaling_groups[0].name
  }
  tags = {
    "KubespotEnvironment" = var.environment_name
  }
...
resource "aws_cloudwatch_metric_alarm" "nodes_blue_cpu_threshold" {
  ...
  dimensions = {
    ...
    AutoScalingGroupName = aws_eks_node_group.blue.resources[0].autoscaling_groups[0].name
  }
  tags = {
    "KubespotEnvironment" = var.environment_name
  }
